...
# Load up our CNN
cnn_model = None
cnn_model = get_cnn_model(SETTINGS['servo_cnn_name'], SETTINGS['width'], SETTINGS['height'], SETTINGS['depth'])
try:    
    cnn_model.load(SETTINGS['servo_cnn_name'] + '.ckpt')
...
def handle_index_page():
    ...
    <html>
        ...
        <body>
            ...
            <button id="recordId" onClick=onRecordButtonClicked(this) style="font-size: 14px">Record</button><br>
            <button id="autodriveId" onClick=onAutodriveButtonClicked(this) style="font-size: 14px">AUTODRIVE-OFF</button><br>
            <table>
                <tr>
                    <td valign = top>
            ...
            <script type="text/javascript">
                ...
                function onRecordButtonClicked(button)
                {
                    postHttpRequest("setRecord", {})
                }

                function onAutodriveButtonClicked(button)
                {
                    gIsAutodrive = !gIsAutodrive
                    if (gIsAutodrive){
                        document.getElementById("autodriveId").innerHTML = 'AUTODRIVE-ON'
                    }else{
...
def handle_cozmoImage():
    ...
    if remote_control_cozmo:
        ...
        if image:
            ...
            remote_control_cozmo.logImages.insert(remote_control_cozmo.fIdx,PIL2array(image).tolist())
            remote_control_cozmo.logServos.insert(remote_control_cozmo.fIdx,remote_control_cozmo.cur_servo)
            remote_control_cozmo.logMotors.insert(remote_control_cozmo.fIdx,remote_control_cozmo.cur_motor)
...
def get_new_filename(path,filenameroot,ext):
    cnt = 0
    while True:
        filename = filenameroot+"_"+str(cnt)
...
@flask_app.route('/setRecord', methods=['POST'])
def handle_setRecord():
    '''Called from Javascript whenever the record button is pressed '''
    # write data to cvs file
    raw_data = {                                                       
        'image': remote_control_cozmo.logImages,
        'servo': remote_control_cozmo.logServos,
        'motor': remote_control_cozmo.logMotors
    }                                                                  
    df = pd.DataFrame(raw_data, columns=['image', 'servo', 'motor'])
    filenameout = get_new_filename(os.path.expanduser('~/suiron/data/'),'cozmoData','.csv')
    df.to_csv(filenameout)
    print(str(filenameout)+' saved')

    # clean the buffer
    remote_control_cozmo.logImages = [];
    remote_control_cozmo.logServos = [];
    remote_control_cozmo.logMotors = [];
        
    return ""

@flask_app.route('/setAutodrive', methods=['POST'])
